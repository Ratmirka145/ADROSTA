...
export function createApp() {
  ...
  app.post("/api/wholesale-order", (request, response) => {
    ...
    try {
      ...
      const orderNumber = createDocumentNumber(
        "ADR",
        order.clientRequestId,
        order.createdAt
      );

      const invoiceNumber = createDocumentNumber(
        "INV",
        order.clientRequestId,
        order.createdAt
      );
      ...
      if (
        ...
        warnings.push(
          "Сумма на клиенте отличалась от серверного расчёта. Использован серверный расчёт."
        );
      }
      ...
      response.status(201).json({
        ...
        stage: "validated",
        message:
          "Заказ принят и пересчитан сервером. PDF и база данных будут подключены следующим этапом.",
        orderNumber,
        invoiceNumber,
        invoiceUrl: null,
